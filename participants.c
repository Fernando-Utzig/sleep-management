...
Operation_result AddParticipantToTable(Participant *participant)
{
    ...
    for(i=0;i<LIST_SIZE;i++)
    {
        if(List.list[i].id != -1 && strcmp(List.list[i].Hostname,participant->Hostname) == 0)//List.list[i].id != -1 && strcmp(List.list[i].Hostname,participant->Hostname) == 0)
        {
